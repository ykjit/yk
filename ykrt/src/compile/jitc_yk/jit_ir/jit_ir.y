...
Inst -> Result<ASTInst, Box<dyn Error>>:
  ...
  | "LOCAL_OPERAND" ":" Type "=" "PARAM" "REG" {
      Ok(ASTInst::Param{assign: $1?.span(), type_: $3?})
    }
  ...
  | "LOCAL_OPERAND" ":" Type "=" "PTR_TO_INT" Operand {
      Ok(ASTInst::PtrToInt{assign: $1?.span(), type_: $3?, val: $6? })
    }
  | "LOCAL_OPERAND" ":" Type "=" "INT_TO_PTR" Operand {
      Ok(ASTInst::IntToPtr{assign: $1?.span(), type_: $3?, val: $6? })
    }
  ...
  | "LOCAL_OPERAND" ":" Type "=" "UI_TO_FP" Operand {
      Ok(ASTInst::UIToFP{assign: $1?.span(), type_: $3?, val: $6? })
    }
