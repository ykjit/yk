...
impl TraceBuilder {
    ...
    /// Check if the condition is a comparison of `__yk_thread_tracing_state` with 0.
    ///
    /// This pattern is generated by the ConditionalPromoteCalls LLVM pass to skip promote calls
    /// when not tracing. We don't want to emit a guard for this check because:
    /// - During tracing: state != 0, so we take the "promote" path (which is what we trace)
    ...
    fn is_tracing_state_check(&self, cond: &aot_ir::Operand) -> bool {
        ...
        let aot_ir::Inst::ICmp { lhs, rhs, .. } = cond.to_inst(self.aot_mod) else {
            return false;
        };

        // Determine which operand is the load and which is the constant.
        let (load_op, const_op) = match (&lhs, &rhs) {
            (aot_ir::Operand::Local(_), aot_ir::Operand::Const(_)) => (lhs, rhs),
            (aot_ir::Operand::Const(_), aot_ir::Operand::Local(_)) => (rhs, lhs),
            _ => return false,
        };

        // Check the constant is 0.
        let aot_ir::Operand::Const(const_idx) = const_op else {
            return false;
        };
        let aot_ir::Const::Val(cv) = self.aot_mod.const_(*const_idx) else {
            return false;
        };
        if !cv.bytes().iter().all(|&b| b == 0) {
            return false;
        }

        // Check the load is from __yk_thread_tracing_state.
        let aot_ir::Inst::Load { ptr, .. } = load_op.to_inst(self.aot_mod) else {
            return false;
        };
        let aot_ir::Operand::Global(gd_idx) = ptr else {
            return false;
        };
        self.aot_mod.global_decl(*gd_idx).name() == "__yk_thread_tracing_state"
    }
