...
pub(crate) enum Inst {
    ...
    Call {
        ...
        // The stackmap generated by ykllvm for calls has an extra element at the end that captures
        // a call's return value. This is the "after" statepoint...
        statepoint_after: Option<Statepoint>,
        // ...and from that "after" statepoint, we can simply strip the last element (if it exists)
        // to create the "on" statepoint.
        #[deku(
            skip,
            default = "statepoint_after.as_ref().map(|x| Statepoint { smapidx: x.smapidx, lives: x.lives[..x.lives.len().saturating_sub(1)].to_vec() })"
        )]
    ...
    IndirectCall {
        ...
        // The stackmap generated by ykllvm for calls has an extra element at the end that captures
        // a call's return value. This is the "after" statepoint...
        statepoint_after: Statepoint,
        // ...and from that "after" statepoint, we can simply strip the last element (if it exists)
        // to create the "on" statepoint.
        #[deku(
            skip,
            default = "Statepoint { smapidx: statepoint_after.smapidx, lives: statepoint_after.lives[..statepoint_after.lives.len().saturating_sub(1)].to_vec() }"
        )]
        statepoint: Statepoint,
    },
    #[deku(id = "16")]
