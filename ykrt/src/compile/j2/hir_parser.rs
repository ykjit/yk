    ...
    fn build(mut self, astexterns: Vec<AstExtern>, astinsts: Vec<AstInst>) -> Mod<Reg> {
        ...
        for inst in astinsts {
            match inst {
                ...
                AstInst::CtTz { local, ty, val } => {
                    self.p_def_local(local);
                    let tyidx = self.p_ty(ty);
                    let val = self.p_local(val);
                    self.insts.push(CtTz { tyidx, val }.into());
                }
                AstInst::Fshl {
                    local,
                    ty,
                ...
                AstInst::SAddOverflow {
                    ...
                    let lhs = self.p_local(lhs);
                    let rhs = self.p_local(rhs);
                    self.insts.push(SAddOverflow { lhs, rhs }.into());
                }
                AstInst::USubOverflow {
                    ...
                    let lhs = self.p_local(lhs);
                    let rhs = self.p_local(rhs);
                    self.insts.push(USubOverflow { lhs, rhs }.into());
                }
                AstInst::SSubOverflow {
                    local,
                    ty,
                    lhs,
                    rhs,
                    ...
                    let lhs = self.p_local(lhs);
                    let rhs = self.p_local(rhs);
                    self.insts.push(SSubOverflow { lhs, rhs }.into());
                }
                AstInst::UMulOverflow {
                    local,
                    ty,
                    lhs,
                    rhs,
                    ...
                    let lhs = self.p_local(lhs);
                    let rhs = self.p_local(rhs);
                    self.insts.push(UMulOverflow { lhs, rhs }.into());
                }
                AstInst::SMulOverflow {
                    ...
                    let lhs = self.p_local(lhs);
                    let rhs = self.p_local(rhs);
                    self.insts.push(SMulOverflow { lhs, rhs }.into());
                }
                ...
                AstInst::UMax {
                    ...
                    let lhs = self.p_local(lhs);
                    let rhs = self.p_local(rhs);
                    self.insts.push(UMax { tyidx, lhs, rhs }.into());
                }
                AstInst::UMin {
                    local,
                    ty,
                    ...
                    let lhs = self.p_local(lhs);
                    let rhs = self.p_local(rhs);
                    self.insts.push(UMin { tyidx, lhs, rhs }.into());
                }
                AstInst::URem {
                    local,
                    ty,
                    ...
                    let lhs = self.p_local(lhs);
                    let rhs = self.p_local(rhs);
                    self.insts.push(URem { tyidx, lhs, rhs }.into());
                }
                AstInst::Xor {
                    local,
                    ty,
