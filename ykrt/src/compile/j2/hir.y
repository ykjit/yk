...
Inst -> Result<AstInst, Box<dyn Error>>:
  ...
  | "LOCAL" ":" Ty "=" "CTTZ" "LOCAL" {
      Ok(AstInst::CtTz { local: $1?.span(), ty: $3?, val: $6?.span() })
    }
  ...
  | "LOCAL" ":" Ty "=" "FNEG" "LOCAL" {
       Ok(AstInst::FNeg { local: $1?.span(), ty: $3?, val: $6?.span() })
    }
  | "LOCAL" ":" Ty "=" "FSUB" "LOCAL" "," "LOCAL" {
       Ok(AstInst::FSub { local: $1?.span(), ty: $3?, lhs: $6?.span(), rhs: $8?.span() })
    }
  | "LOCAL" ":" Ty "=" "FPCLASS" "LOCAL" "," "INT" {
      Ok(AstInst::FPClass { local: $1?.span(), ty: $3?, val: $6?.span(), test: $8?.span() })
    }
  ...
  | "LOCAL" ":" Ty "=" "UMIN" "LOCAL" "," "LOCAL" {
       Ok(AstInst::UMin { local: $1?.span(), ty: $3?, lhs: $6?.span(), rhs: $8?.span() })
    }
