...
impl InstT for USubOverflow {
    ...
    fn cse_eq(&self, opt: &dyn EquivIIdxT, other: &Inst) -> bool {
        if let Inst::USubOverflow(USubOverflow { tyidx, lhs, rhs }) = other
            && self.tyidx == *tyidx
            && opt.equiv_iidx(self.lhs) == *lhs
            && opt.equiv_iidx(self.rhs) == *rhs
        {
...
impl InstT for SSubOverflow {
    ...
    fn cse_eq(&self, opt: &dyn EquivIIdxT, other: &Inst) -> bool {
        if let Inst::SSubOverflow(SSubOverflow { tyidx, lhs, rhs }) = other
            && self.tyidx == *tyidx
            && opt.equiv_iidx(self.lhs) == *lhs
            && opt.equiv_iidx(self.rhs) == *rhs
        {
...
/// Return the unsigned maximum of two values, with the same semantics as LLVM's `llvm.umax`.
#[derive(Clone, Debug)]
pub(super) struct UMax {
    pub tyidx: TyIdx,
    /// What LLVM calls `a`.
    pub lhs: InstIdx,
    /// What LLVM calls `b`.
...
impl InstT for UMax {
    ...
    fn cse_eq(&self, opt: &dyn EquivIIdxT, other: &Inst) -> bool {
        if let Inst::UMax(UMax { tyidx, lhs, rhs }) = other
            && self.tyidx == *tyidx
            && opt.equiv_iidx(self.lhs) == *lhs
            && opt.equiv_iidx(self.rhs) == *rhs
        {
